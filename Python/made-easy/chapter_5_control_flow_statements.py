...
# начинается со следующей строки и должен иметь отступ. Отступ сообщает Python
...
# блок кода предназначен для оператора if. Затем идет ключевое слово else на
# новой
# строке без отступа, за ним следует двоеточие. Блок кода оператора else
# начинается
...
# после него следует соответствующий блок операторов (разумеется, с правильным
# отстулом). Оператор if можно использовать внутри блока if другого оператора
# if это называется вложенный оператор if.
#
...
# экономя
# память. Подобрые объекты называются итерируемыми, т.е. их можно обрабатывать
# с помощью функций и конструкций, которые ожидают получить тип данных, у
# которого
...
# шается, перебрав все элементы (в случае цикла for) или когда условие
# становится
# ложным (в случае цикла while). Блок else не выполняется, когда цикл
# завершается
# оператором break.
#
...
# остановил цикл for и не перевел его на следующую итерацию и пропустил блок
# else.
...
# Оператор continue используется для пропуска части кода внутри цикла в
# пределах
...
# выполнять не нужно. Еще его можно использовать в качестве заполнителя для
# функции или тела условия
# в момент написания нового кода. Эrо позволяет сначала набросать шаблон кода
# на
