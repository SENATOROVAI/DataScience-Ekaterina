...
# классифицирует города по количеству населения на 3 категории: А, В и С.
# Размер
...
city_class = input(f"{name} живет в городе класса: ")
...
# хотим использовать (w или i в приведенных примерах), а затем используем
# опера-
# тор in, чтобы связать переменную с последовательностью, которую мы хотим
# пере-
...
# становится
# ложным (в случае цикла while). Обратите внимание, что блок else не
# выполняется,
...
# оператор break остановил цикл for и не перевел его на следующую итерацию и
# про
# пустил блок else.
...
# пределах
# текущей итерации (рис. 5.9). Цикл не завершается, а сразу перейдет на
# следующую
# итерацию
...
# class MyEmptyClass:
# 			pass
# Еще его можно использовать в качестве заполнителя для функции или тела
# условия
